...
const double TU134_MASS = 47000.0;          // Масса самолета, кг
const double TU134_WING_AREA = 127.0;       // Площадь крыла, м²
const double TU134_NOMINAL_THRUST = 2 * 68000.0; // Суммарная тяга, Н
const double TU134_CY0 = 0.25;               // Коэффициент подъемной силы при нулевом угле атаки
...
const double START_VELOCITY = 320.0 / 3.6; // Начальная скорость, м/с
const double FINAL_VELOCITY = 720.0 / 3.6;  // Конечная скорость, м/с
...
const double MIN_SPEED = START_VELOCITY; // Минимальная скорость для набора высоты
...
public:
    ...
    void plotTrajectory() const {
        ...
        fprintf(gp, "set multiplot layout 2,2 title 'Параметры полета Tу-134'\n");
...
public:
    ...
    Aircraft(double m = TU134_MASS, double wing = TU134_WING_AREA)
        : wing_area(wing), initial_mass(m), env(), mass(m) {

        thrust = TU134_NOMINAL_THRUST * MAX_THRUST_PERCENT;
        //fuel_flow = 2.5; // кг/с - при крейсерском движении
        Cx0 = 0.0275;
        K = 0.05;
        Cy_max = 1.2;
    }
    ...
    double calculate_alpha(double current_altitude, double current_velocity) {
        ...
        double alpha = (TU134_MASS * GRAVITY - q * TU134_WING_AREA * TU134_CY0) / (q * TU134_WING_AREA * grad_Cy + current_P);
        return alpha;

    }
    // Расчет градиента коэффициента подъема
    double grad_Cy_alpha() const{
        double b = 33.75; // размах крыла
        double AR = b*b/TU134_WING_AREA;
...
double getLiftCoefficient(double alpha) const{
    ...
    double computeLiftForce(double current_velocity, double current_altitude, double alpha) const {
        ...
        double L = 0.5 * rho * current_velocity * current_velocity * TU134_WING_AREA * Cy;
        return L;
    }
    ...
    double computeDragForce(double current_velocity, double current_altitude, double alpha) const {
        ...
        double X = 0.5 * rho * current_velocity * current_velocity * TU134_WING_AREA * Cx;
        return X;
    }
    ...
    double total_thrust(double current_altitude){
        ...
        return TU134_NOMINAL_THRUST*MAX_THRUST_PERCENT * pow(current_p/p_0, 0.7); // коррекция тяги по плотности и давлению
    }
};
...
public:
    ...
    double calculate_razgon(double altitude, double initial_velocity, double final_velocity, Aircraft& ac){
        ...
        double a_x = ((current_P * cos(alpha_rad)) - q * Cx * TU134_WING_AREA) / TU134_MASS;
        //std::cout<<"ускорение ах"<<a_x<<std::endl;
    ...
    double calculate_podiem(double initial_alt, double final_alt, double velocity, Aircraft& ac){
        ...
        double X = q * TU134_WING_AREA * Cx;

        double sin_tetha = std::min((current_P-X) / (TU134_MASS* GRAVITY), MAX_CLIMB_ANGLE/DEG_TO_RAD);
        double vel_y = velocity * sin_tetha;
        if (vel_y > MAX_VERTICAL_SPEED) vel_y = MAX_VERTICAL_SPEED;

        double dt = (final_alt-initial_alt) / vel_y;
        //std::cout<<"вертик скорость"<<vel_y<<std::endl;
        return dt;
    }
...
int main() {
    ...
    try {
        ...
        Trajectory trajectory = solver.computeOptimalTrajectory(tu134);
        trajectory.saveToCSV("tu134_trajectory_realistic.csv");
